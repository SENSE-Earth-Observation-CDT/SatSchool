...
def on_refresh_click(book_id):
    return

def on_next_click(book_id):
    """
    ...
    """
    st.session_state.file_number[book_id] = (st.session_state.file_number[book_id] + 1) % st.session_state.total_files
    return

def on_previous_click(book_id):
    """
    Updates the values of update_from_selectbox and update_from_button.
    Updates the page number to +1, or 0 if the last page was reached.
    """
...
def create_buttons(caption_text, 
                    ...
                    button_next, 
                    button_refresh,
                    username, repository, book_id, key=""):
    """
    ...
    c1.button(button_previous, 
                help="Previous page", on_click=on_previous_click, args=([book_id]), key="previous_button_top"+key)
    c2.button(button_next, 
                help="Next page", on_click=on_next_click, args=([book_id]), key="next_button_top"+key)
    c3.button(button_refresh, 
                help="Refresh current page", on_click=on_refresh_click, args=([book_id]), key="switch_button_top"+key)
...
def set_book_config(path,
    ...
    if book_id not in st.session_state.file_number:
    	st.session_state.file_number[book_id] = 0
    ...
    selected_file_fullpath = file_list[st.session_state.file_number[book_id]]
    #caption_text = f"Page {st.session_state.file_number[book_id]+1} of {st.session_state.total_files}. File: {selected_file_fullpath}"
    caption_text = f"Page {st.session_state.file_number[book_id]+1} of {st.session_state.total_files}."

    if st.session_state.toc:
        option = st.radio("Table of contents", options=file_list)
        st.session_state.file_number[book_id] = file_list.index(option)
        ...
        if st.session_state.button in ["top", "both"]:
            create_buttons(caption_text, 
                            ...
                            username, repository, book_id, key="top")
        ...
        if st.session_state.button in ["bottom", "both"]:
            create_buttons(caption_text, button_previous, button_next, button_refresh, 
                            username, repository, book_id, key="bottom")
